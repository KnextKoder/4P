...
export default function FourPicsOneWord() {
  ...
  }, [selectedAnswer, availableLetters])
  ...
  }, [gameData, availableLetters, selectedAnswer, isCorrect, selectLetter, removeLetter, checkAnswer])
  ...
  return (
    <div className="min-h-[calc(100vh-56px-56px)] relative p-2 sm:p-4">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="mb-3 sm:mb-4">
          <Button
            variant="outline"
            onClick={() => router.push('/')}
            className="bg-white/10 border-white/20 text-white hover:bg-white/20 shadow-sm text-sm sm:text-base"
          >
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 mb-3 sm:mb-4">
          <div className="flex items-center gap-2 rounded-full bg-yellow-400/10 ring-1 ring-yellow-300/30 px-3 py-1.5 self-start">
            <Trophy className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-300" />
            <span className="text-white font-bold text-base sm:text-lg leading-none">{score}</span>
          </div>
          {gameData?.answer && (
          ...
          <Button
            ...
            onClick={useHint}
            disabled={score < 50}
            className="bg-white/10 border-white/20 text-white hover:bg-white/20 disabled:opacity-50 text-sm self-start sm:self-auto"
          >
            <Lightbulb className="w-3 h-3 sm:w-4 sm:h-4 mr-1 text-yellow-300" />
            Hint
        ...
        {/* Images Grid */}
        <div className="grid grid-cols-2 gap-2 sm:gap-4 mb-4 sm:mb-6">{gameData.image_data && gameData.image_data.length > 0 ? (
            ...
            Array.from({ length: 4 }, (_, index) => (
              <div
                key={index}
                className="rounded-lg border border-white/10 bg-white/5 h-32 sm:h-40 overflow-hidden relative"
              >
        ...
        {/* Answer Slots */}
        <div className={`flex justify-center gap-1.5 sm:gap-2.5 mb-4 sm:mb-6 ${isWrong ? "animate-shake" : ""} overflow-x-auto px-2`}>
          {selectedAnswer.map((letter, index) => (
            <Button
              key={index}
              variant="outline"
              className={`w-10 h-10 sm:w-14 sm:h-14 text-lg sm:text-2xl font-bold border-2 rounded-md shadow-sm transition-colors flex-shrink-0 ${
                isWrong
        ...
        {/* Available Letters */}
        <div className="grid grid-cols-6 gap-1.5 sm:gap-2.5 mb-16 sm:mb-24">
          {availableLetters.map((letter, index) => (
            <Button
              key={index}
              variant="secondary"
              className="h-10 sm:h-14 text-lg sm:text-xl font-bold bg-white text-slate-900 hover:bg-white/90 active:translate-y-[1px] border border-white shadow-sm"
        ...
        <div className="fixed left-0 right-0 bottom-16 md:static md:bottom-auto md:mb-6 flex flex-col sm:flex-row gap-2 sm:gap-4 justify-center px-3 sm:px-4 py-3 bg-slate-900/60 supports-[backdrop-filter]:bg-slate-900/30 backdrop-blur border-t border-white/10">
          <Button
            onClick={checkAnswer}
            disabled={selectedAnswer.includes("") || isCorrect}
            className="bg-green-500 hover:bg-green-600 text-white px-6 sm:px-8 py-2 sm:py-3 text-base sm:text-lg border border-white/10 shadow"
          >
            Check Answer
          </Button>
          <Button
            variant="outline"
            onClick={resetLevel}
            className="bg-white/10 border-white/20 text-white hover:bg-white/20 px-4 sm:px-6 py-2 sm:py-3"
          >
        ...
        {/* Success Message */}
        {isCorrect && (
          <div className="fixed inset-0 bg-orange-950/70 backdrop-blur-sm flex items-center justify-center z-50 p-3 sm:p-4">
            {/* Confetti */}
            ...
            <Card className="max-w-sm sm:max-w-md w-full mx-auto border-white/20 bg-orange-500/20 text-white shadow-2xl">
              <CardContent className="p-4 sm:p-6 text-center">
                <h2 className="text-xl sm:text-2xl font-bold text-white mb-3">Ẹ kú se! 🎉</h2>
                <div className="bg-orange-500/10 border border-orange-400/30 rounded-lg p-3 sm:p-4 mb-4">
                  <div className="text-center">
                    <p className="text-base sm:text-lg font-bold text-orange-100 mb-2">
                      <span className="text-xl sm:text-2xl">{gameData.answer}</span>
                    </p>
                    <p className="text-orange-200 text-xs sm:text-sm mb-1">
                ...
                {gameData.educational_fact && (
                  <div className="bg-blue-500/10 border border-blue-400/30 rounded-lg p-3 sm:p-4 mb-4 text-left">
                    <h3 className="text-blue-200 font-semibold mb-2 flex items-center text-sm sm:text-base">
                ...
                <div className="flex justify-center">
                  <Button onClick={playAgain} className="bg-orange-600 hover:bg-orange-700 text-white text-sm sm:text-base px-4 sm:px-6 py-2 sm:py-3">
                    Learn More
                  </Button>
                </div>
              </CardContent>
        ...
        {/* Error Message */}
        {isWrong && (
          <Card className="bg-red-500/10 border-red-400/40 mb-4 sm:mb-6 animate-pulse">
            <CardContent className="p-4 sm:p-6 text-center">
              <h2 className="text-lg sm:text-2xl font-bold text-red-200 mb-2">Wrong Answer! ❌</h2>
              <p className="text-red-200/90 mb-2 text-sm sm:text-base">
                <strong>&quot;{wrongAnswer}&quot;</strong> is not correct.
              </p>
              <p className="text-red-200/80 text-xs sm:text-sm">Try again! Look carefully at the images.</p>
            </CardContent>
          </Card>
        )}
        ...
        {/* Hint Display */}
        {showHint && (
          <Card className="bg-blue-500/10 border-blue-400/30 mb-4 sm:mb-6">
            <CardContent className="p-3 sm:p-4 text-center">
              <p className="text-blue-100 text-sm sm:text-base">💡 Hint: Think about what all four images have in common!</p>
            </CardContent>
          </Card>
        )}
      </div>
